...
for every topic is the most expensive thing it serves. A dashboard on a short
refresh would throttle itself and then report an outage it had caused. So
results are cached (`UCONNECT_CACHE_TTL`, default 5s), and the page refreshes
on a plain interval rather than holding a connection open.
...
- LOOKUP needs no key. `K` never reaches the server and is only ever used
  between peers, so knowing a `topic_id` is sufficient — which is how every
  peer already finds the others.
- Metadata is plaintext by design.

So this makes existing exposure visible; it does not create any. That also
means the lookup box works for unlisted topics: "unlisted" means absent from
the directory, not secret.
...
| `UCONNECT_REFRESH` | `15` | page auto-refresh seconds, 0 to disable |
| `UCONNECT_SHA_FILE` | `/opt/uconnect/deployed.sha` | deployed commit |
...
UCONNECT_SERVER=1.2.3.4:4433 gunicorn -w 1 --threads 8 -b 127.0.0.1:8080 --chdir web app:app
```

One worker, several threads: workers do not share memory, and four of them
would mean four independent caches and four times the lookups.
...
will not start — all log loudly and leave the server alone. A broken dashboard
is an inconvenience; a rolled-back rendezvous server is an outage.

It watches `127.0.0.1:4433`, so its traffic never leaves the box, and its
lookups are rate-limited under the loopback address rather than the public one
— a busy dashboard cannot throttle real peers.
...
`python3 -m venv --help` succeeds even when venv creation cannot, so both the
installer and the watcher build a real one and print the actual error.
...
at the subprocess seam: caching, stale-on-failure, topic-id validation, the
derived statistics, and the error messages for a missing binary or unparseable
output.
